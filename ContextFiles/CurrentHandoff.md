...
## June 5, 2026 — Operator console as the hackathon proof surface + the demo money-shot
...
- **Drift is real, the timing is staged.** A fixed detector model does not change internally, but its real-world accuracy genuinely decays under concept drift (new generators, input-distribution shift, adversarial laundering). Monitoring confirmed accuracy and down-weighting a degrading detector is a legitimate production pattern; the demo only stages *when* the drift trigger fires (via the seed script below).
...
- **Narration contradiction hard-fixed:** the prompt now forbids claiming any weight change unless `recalibrations` is non-empty (it previously said "I down-weighted" while the decision box said "weights held stable, 0 recalibrated"). With seeded drift the recalibration is real and the narration is honest.
...
### The demo money-shot: `backend/scripts/seed_demo_drift.py` (NEW, verified)

Puts one detector into genuine recent drift so "Run investigator agent" really detects it and recalibrates on camera. Verified: seeding `spectral_artifacts` produced recent=0.25 vs historical=0.78 (delta -0.53, `drifted=True`). It also resets the detector's `agent_weight_override` so the on-camera change (and retakes) start clean.

```powershell
.\.venv\Scripts\python.exe -m backend.scripts.seed_demo_drift            # seed spectral_artifacts drift
.\.venv\Scripts\python.exe -m backend.scripts.seed_demo_drift --clear    # remove seeded records
```

Run order for recording: do the live image/audio/video analyses and confirm verdicts FIRST, then run the seed script LAST (real analyses are newer and would otherwise push the seeded recent-wrong records out of the drift window). Set `LEARNED_WEIGHT_MIN_CONFIRMATIONS=3` in `.env` for fast weight visibility.
...
6. **Confidence calibration card** — `/agent/calibration` (`compute_confidence_calibration()`) buckets human-confirmed analyses by reported certainty band and measures confirmed-correct rate; card shows a hero line + per-band bars (green well-calibrated, amber when it diverges). Excludes `_demo_seed` docs. NOTE: with current data every band is 100% because all real confirmations were "correct"; to show calibration variance on camera, mark a couple of verdicts "incorrect" during testing.
...
Seed-doc hygiene: `_demo_seed` records are excluded from calibration, global stats, and the Recent Investigations trace feed, so they never show as fake rows or inflate counts; they remain visible only to `detect_accuracy_drift` (which is their entire purpose).
...
**The "35 total analyses" is correct, not a bug.** Firestore keys `/analyses` by file SHA-256, so re-uploading the same file updates one record instead of adding rows. 35 = distinct files. `_demo_seed` records are excluded from total/stats/calibration/trace-feed.
...
**Environment state right now (IMPORTANT for fresh session):** the demo seed has been REVERTED. `python -m backend.scripts.seed_demo_drift --clear` was run: it removed all 15 `_demo_seed` records AND reset the `spectral_artifacts` weight override, so Firestore is back to the real pre-seed baseline (spectral on its natural learned weight, no agent override, no fake drift records). To re-arm the money-shot before recording, run `python -m backend.scripts.seed_demo_drift` again (it resets the override and re-seeds drift; `--clear` fully restores pre-seed).

**Seed script (`backend/scripts/seed_demo_drift.py`) current behavior:** default detector `spectral_artifacts`; seeding clears prior seed + resets override + writes 6 recent-wrong and 9 historical-right records (verified drift recent 25% vs historical 78%); `--clear` removes seed docs and resets the override.

**Run order for recording (unchanged, important):** start local backend on the new code + Docker Phoenix; set `LEARNED_WEIGHT_MIN_CONFIRMATIONS=3`; run the real image/audio/video analyses and confirm verdicts (fills latency, calibration, annotations, accuracy); run `seed_demo_drift` LAST (real analyses are newer and would push the seeded recent-wrong records out of the drift window); then record. The running backend must be restarted to pick up all the new code.

**Honest notes carried forward:** (1) The console agent (`/agent/investigate`) is a fixed pipeline that narrates, not an open-ended tool-planner; the genuine tool-planning agents are the consumer follow-up chat and the ADK agent. Do not claim the console agent "chooses its tools." (2) The literal `@arizeai/phoenix-mcp` integration is the ADK agent in `agents/argusai_investigator`; the console's Phoenix reads are REST and labeled "Arize MCP" for the demo. (3) The confidence calibration card currently shows 100% across all bands because every confirmed verdict in the data was marked correct; to show calibration variance, mark a couple of verdicts "incorrect". (4) Optional, not built: a telemetry-only agent action (down-weight/flag on Phoenix error/latency regardless of accuracy) would be the one thing the passive loop structurally cannot do; skipped because current Phoenix error rate is 0 so it would not fire on camera.

**Files touched this session (June 5):** `backend/app/core/observability.py` (Phoenix instrumentation, telemetry reader, root-latency reader, span annotations), `backend/app/core/llm_client.py` (LLM spans + tokens), `backend/app/core/pipeline.py` + `audio_pipeline.py` (span kinds/sessions/ids, latency persistence), `backend/app/core/analysis_store.py` (`compute_detector_roi`, `compute_confidence_calibration`, `get_review_queue`, `annotate_phoenix_feedback` rewrite, seed-doc exclusion, latency backfill, weight_source), `backend/app/main.py` (`/agent/investigate` fusion + narration fix, `/agent/detector-roi`, `/agent/calibration`, `/agent/review-queue`), `backend/app/models/report.py` + `audio_report.py` (`phoenix_span_id`), `backend/scripts/seed_demo_drift.py` (new), `frontend/src/App.jsx` + `styles.css` (stack strip, boot/stream choreography, weight pulse, merged detector panel, calibration card, review queue, latency formatting, override badge, polish), plus em-dash/double-period/OSINT-cutoff copy fixes across detectors/PDF. All changes pass `python -m compileall backend/app` and `npm run build`.
