...
3. Per-detector **confirmed accuracy** is tracked in Firestore (`detector_stats.confirmed_total / confirmed_correct / confirmed_accuracy`), and **per media type** in `detector_stats.confirmed_by_media` ({image/video/audio: total, correct, accuracy}). This is distinct from the older `accuracy_rate`, which is *self-agreement* (how often a detector sided with the system's own verdict) and is therefore circular — confirmed accuracy vs. human truth is the number that actually means something.
4. `get_learned_weights(media_type)` converts confirmed accuracy into a per-detector weight multiplier, **media-aware** with fallback: agent override → media-specific confirmed accuracy (once it has `LEARNED_WEIGHT_MIN_CONFIRMATIONS`, env-tunable, default 8) → global confirmed accuracy → 1.0x. Bounded 0.5x–1.5x, with 0.6 confirmed accuracy mapping to 1.0x. Media-aware because a detector behaves differently on image vs video vs audio.
5. The verdict engine (`backend/app/reasoning/engine.py` `_score_signal`) multiplies each detector's `base_weight` by that learned multiplier, on top of a *media relevance prior* (`MEDIA_IMPORTANCE_OVERRIDE` — e.g. audio is supporting-only for video). Relevance (which signals answer the question) and accuracy (how right they've been) are separate axes that compose. So observability is **causal to the output** — the exact Arize partner-track thesis, made real rather than decorative.
...
The learned spectral model and the measured signals (acoustic micro-signature, sensor noise coherence) target fundamental, hard-to-fake tells. **Metadata is now real provenance**: it scans raw bytes for C2PA / Content Credentials and the standardized AI source type `trainedAlgorithmicMedia` (works on stripped EXIF and on video), so it catches Sora/Veo/Gemini/Firefly provenance. The remaining heuristics (ELA, lighting, basic noise) are kept for breadth but are weak for generation detection and often return inconclusive — the media-aware self-calibration loop surfaces and down-weights them empirically over real use. No signal is unfakeable; reliability comes from breadth + provenance (C2PA + OSINT) + honest "inconclusive" + the confirmed-accuracy loop, not from any single check. Still missing (advice only, not built): diffusion reconstruction error for images, proper PRNU, DCT/quantization forensics, and rPPG for face video. ELA is the wrong tool for generation (it detects compositing) and is intentionally low-weight.
